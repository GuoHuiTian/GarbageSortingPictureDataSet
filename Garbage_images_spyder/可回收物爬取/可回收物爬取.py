...
class BaiduImageSpider(object):  # 创建一个类
    def __init__(self):
        ...
        self.directory = r"F:\Python\爬虫代码\可回收物_{}"  # 存储目录  这里需要修改为自己希望保存的目录  {}不要丢
    ...
    def run(self,name):
        ...
        for index in range(self.json_count):
            ...
            for link in list_image_link:
                pic_number += 1
                self.save_image(link, self.directory.format("img1_"+name+"_"+str(pic_number) + '.jpg'))
...
if __name__ == '__main__':  # 代码作为模块，在别的文件调用时，不会直接运行整个脚本
    filename = open("./可回收物类型.txt", encoding="utf-8")
    str_all = filename.readlines()
    for i in str_all:
        line = i.strip("\n")
        print("正在爬取" + line + "类型的图片")
        spider = BaiduImageSpider()
        spider.json_count = 7 # 默认下载一组图片（30张）
        spider.run(line)
    filename.close()
